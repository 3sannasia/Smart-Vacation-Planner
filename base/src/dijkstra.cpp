...
void dijkstra::dijkstra_distance() {  // for one or two stops
    ...
    for (unsigned int i = 0; i < current_airports.size(); i++) {
        ...
        if (find_path(stops, current_airports, current_airports.at(i).first) != pair<string, int>("", 0)) {
            shortest_paths.push_back(find_path(stops, current_airports, current_airports.at(i).first));
        }
    }
